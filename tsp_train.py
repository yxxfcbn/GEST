...
if __name__ == "__main__":
    ...
    epochs = 50
    print(f"\n开始在训练集上训练，在验证集上验证 ({epochs} epochs)...")
    ...
    # 保存模型供 SCIP 求解脚本使用（命名保留历史，可按需重命名）
    model_save_path = os.path.join(SCRIPT_DIR, "tsp_gnn_model.pt")
    torch.save(model.state_dict(), model_save_path)
    print(f"model save: {model_save_path}")

    # 二阶段稀疏化：现在所有 Data 都已经是“粗筛图版”（只包含 Delaunay+MST 的边）。
    # 因此这里只需要在“粗筛图输出”上扫阈值 p，得到最终稀疏图。
